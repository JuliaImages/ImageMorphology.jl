...
@testset "mgradient" begin
    ...
    for T in test_ranges
        for N in (1, 2, 3)
            ...
            out = mgradient(img)
            @test out == mgradient(img, strel_box(img, ntuple(identity, N); r=1))
            @test out == beucher_gradient_ref(img, ntuple(identity, N), 1)

            @test mgradient(img; dims=(1,), r=2) == beucher_gradient_ref(img, (1,), 2)

            se = rand_se_mask(3, N; symmetric=true)
            @test mgradient(img, se) == beucher_gradient_ref(img, se)
        end
    end
